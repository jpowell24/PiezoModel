...
int counter(double p){
    double time = -pow(p,-1)*log((float) rand()/RAND_MAX);
    //cout << "generates " << -pow(0.5,-1) << " * " << log((float) rand()/RAND_MAX) << " causes " << time << endl;
    //cout << time << endl; 
    int temp = (int)round(time);
    cout << p << " " << temp << endl;
    return(temp);
}

double Piezo_single_channel2(int x){
    double p;
    ...
    int closed_time = counter(P_open(0));
    int open_time = -1; 

    // cout << "initial condition " << closed_time << endl;
    for(double time = 0; time < 800; time+=delta_T){
        ...
        p = vec_open1[time] + vec_inactive_held[time];

        if(time == 200 || time == 600){
            counter_counter = 0; 
            closed_time = counter(1-p);
            open_time = -1; 
        }
        
        if(counter_counter < closed_time){
            // cout << "closed," << closed_time << " ";
            if(x == 0){
        ...
        else if(counter_counter == closed_time){
            open_time = counter(1-p);
            if(x == 0){
            vec_Channel0.push_back(0);
            }
            else if(x == 1){
        ...
        else if(counter_counter < open_time){
            // cout << "open," << closed_time << " ";
            if(x == 0){
            vec_Channel0.push_back(1);
            }
            else if(x == 1){
                vec_Channel1.push_back(1);
            }
            else if(x == 2){
                vec_Channel2.push_back(1);
            }
            else if(x == 3){
                vec_Channel3.push_back(1);
            }
            else if(x == 4){
                vec_Channel4.push_back(1);
            }
            else if(x == 5){
                vec_Channel5.push_back(1);
            }
            else if(x == 6){
                vec_Channel6.push_back(1);
            }
            else if(x == 7){
                vec_Channel7.push_back(1);
            }
            else if(x == 8){
                vec_Channel8.push_back(1);
            }
            else if(x == 9){
                vec_Channel9.push_back(1);
            }
            else{
                cout << "There is an error" << endl;
            }
        ...
        counter_counter++;
        // cout << time << " vs " << counter_counter << endl;
    }
